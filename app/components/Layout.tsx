...
export function Layout(props: { children: React.ReactNode }) {
  ...
  const avatar = user?.avatar === null || !user?.avatar ? undefined : `/images/${user.avatar}`
  const twitter = user?.twitter === null || !user?.twitter ? undefined : user.twitter
  const setting = `/auth/${userid}/setting`
  const edit = `/auth/${userid}/projects`
          ...
          <nav className="flex justify-between items-center px-2 bg-primary">
            ...
            <div className="flex items-center">
              {isLoggedIn ? (
                ...
                <>
                  <NavLink to="/signup">ユーザー登録</NavLink>
